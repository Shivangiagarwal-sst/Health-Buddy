...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('section-yoga').addEventListener('click', e => {
        ...
        if (!pose) return;
        startYoga(pose.dataset.name, pose.dataset.how);
    });
    ...
    document.getElementById('breath-toggle').addEventListener('click', toggleBreathing);
    document.getElementById('dance-chill').addEventListener('click', () => startDance('Chill'));
    document.getElementById('waterCircle').addEventListener('click', addWater);
...
function startDance(type) {
    ...
    const words = ['VIBE!', 'POWER!', 'DANCE!', 'MOVE!'];
...
function startYoga(name, how) {
    ...
    document.getElementById('ex-how-to').innerHTML = `<b>GUIDE:</b><br>${how}`;
...
function updateWaterUI() {
    ...
    document.getElementById('water-count').innerText = `${waterCount} / 8 Glasses`;
}
function launchCelebration() {
    const c = document.getElementById('balloon-container');
    c.innerHTML = '';

    const colors = ['#00f5d4', '#ffcf33', '#f15bb5', '#fee440', '#00b4d8'];
...
function stopEverything() {
    ...
    document.getElementById('breath-box').classList.remove('inhale-act');
    document.getElementById('breath-toggle').innerText = 'Start Session';
}
